// Só faz sentido em projeto com Supabase. O que faz: cruza as colunas e
// tabelas que o CÓDIGO usa contra um retrato do banco real
// (tests/_schema-snapshot.json). Pega o erro mais silencioso deste padrão —
// o código pede uma coluna que não existe e a tela só mostra vazio, sem erro
// nenhum. É estático: não precisa de internet nem de chave.
//
// LIMITAÇÃO CONHECIDA — não enxerga colunas de insert()/update(), só as de
// select() e as dos filtros. Por isso sempre cheque o `error` dos inserts.
//
// Depois de QUALQUER migration, regenere o snapshot — senão este teste passa
// a mentir.
...
for (const f of files) {
  ...
  while ((m = reSelect.exec(txt))) {
    ...
    for (let tok of toks) {
      tok = tok.trim()
      if (!tok || tok === '*' || tok.includes('(')) continue
